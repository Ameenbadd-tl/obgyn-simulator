...
# 5. إدارة الجلسة والذاكرة في المتصفح للحفاظ على سير الحالة
...
if st.session_state.case_started:
    ...
    if st.button("📊 إنهاء الحالة وطلب التقييم من البروفيسور"):
        with st.spinner("جاري تحليل الـ History وإعداد تقرير اللجنة الطبية..."):
            response_text = ask_gemini_audio(text_input="انتهي من تقمص الدور الآن، واكتب لي التقييم الطبي الكامل للحالة باللغة العربية الفصحى، واذكر ما هو التشخيص السري للحالة التي تم اختيارها، وما هي الأسئلة الهامة السريرية التي نسي الطبيب طرحها وما كان يجب عليه التركيز فيه.")
